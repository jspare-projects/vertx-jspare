...
public VertxBuilder deployVerticle(Verticle verticle, DeploymentOptions deploymentOptions){
	...
	private void collectAndRegistryVerticles() {
		...
		// Check if default package are available to scan and add to eventBusPackages
		if (scanDefaultVerticles && source != null) {
		
			String cPackage = source.getClass()
					.getPackage().getName()
					.concat(defaultPackageToScanVerticles)
					.concat(ClasspathScannerUtils.ALL_SCAN_QUOTE);
	
			verticlesPackages.add(cPackage);
		}
		
		// Iterate eventBusPackages scannig and adding classes to eventBusClasses
	...
	private void collectAndRegistryEventBusControllers() {
		...
		if (scanDefaultConsumers && source != null) {
			...
			String cPackage = source.getClass()
					.getPackage().getName()
					.concat(defaultPackageToScanConsumers)
					.concat(ClasspathScannerUtils.ALL_SCAN_QUOTE);
		...
		consumers.forEach(md ->
			eventBus.consumer(md.name(), md.wrap())
		);
	}
